...
class AutobookSupabaseUser extends BaseAuthUser {
  AutobookSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> autobookSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AutobookSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
